...
void C图形::f初始化(HWND a窗口) {
	...
	m总矩阵.m投影 = v投影.ft矩阵4r();
	v缓冲工厂.f创建缓冲(m固定0, &m总矩阵, sizeof(S总矩阵), 三维::E缓冲::e固定);
	v渲染控制.fs固定缓冲(0, m固定0.Get());
	//固定1
	m光.m位置 = 数学::S向量3::c零;	//光源在 f计算 中不断变化
	m光.m颜色 = 数学::S颜色::c白;
	m光.m开始 = 2;
	m光.m范围 = 4;
	v缓冲工厂.f创建缓冲(m固定1, &m光, sizeof(S光), 三维::E缓冲::e固定);
	v渲染控制.fs固定缓冲(1, m固定1.Get());
	//其它状态
...
void C图形::f更新() {
	auto &v渲染控制 = m三维.fg渲染控制();
	m总矩阵.m世界 = m世界.ft矩阵4();
	v渲染控制.f更新资源(m固定0.Get(), &m总矩阵);
	v渲染控制.f更新资源(m固定1.Get(), &m光);
}
void C图形::f显示() {
	auto &v渲染控制 = m三维.fg渲染控制();
	v渲染控制.f清屏();
