...
void C图形::f初始化(HWND a窗口) {
	...
	m总矩阵.m视图 = v相机.ft矩阵4r();
	数学::S透视投影 v投影;
	v投影 = 数学::S透视投影(m窗口大小.x, m窗口大小.y);
	m总矩阵.m投影 = v投影.ft矩阵4r();
	v缓冲工厂.f创建缓冲(m常量0, &m总矩阵, sizeof(S总矩阵), 三维::E缓冲::e常量);
	v渲染控制.fs常量缓冲(0, m常量0.Get());
	//常量1
	m光.m位置 = 数学::S向量3::c零;	//光源在 f计算 中不断变化
	m光.m颜色 = 数学::S颜色::c白;
	m光.m开始 = 2;
	m光.m范围 = 4;
	v缓冲工厂.f创建缓冲(m常量1, &m光, sizeof(S光), 三维::E缓冲::e常量);
	v渲染控制.fs常量缓冲(1, m常量1.Get());
	//其它状态
...
void C图形::f计算() {
	...
	m世界.m旋转.y = m秒 * 0.5f;
	m世界.m旋转.z = m秒 * 0.2f;
	const 数学::S向量2 v光旋转 = 数学::S向量2::fc方向r(4, m秒);
	m光.m位置 = 数学::S向量3(v光旋转.x, 4, v光旋转.y);
}
void C图形::f更新() {
	auto &v渲染控制 = m三维.fg渲染控制();
	m总矩阵.m世界 = m世界.ft矩阵4();
	v渲染控制.f更新资源(m常量0.Get(), &m总矩阵);
	v渲染控制.f更新资源(m常量1.Get(), &m光);
}
void C图形::f显示() {
	auto &v渲染控制 = m三维.fg渲染控制();
	v渲染控制.f清屏();
